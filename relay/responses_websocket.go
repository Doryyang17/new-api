...
func (s *responsesWSSession) runRequest(state *responsesWSCallState, message []byte, envelope responsesWSCreateEvent, streamID string, requestID string) {
	...
	apiErr = s.runner(request, requestID, func(c *gin.Context) *types.NewAPIError {
		...
		if middleware.CheckSystemModelDailyUsageLimit(c, create.Request.Model) {
			return nil
		}
		middleware.PromptComplianceCheck("/v1/responses")(c)
		if c.IsAborted() {
			return nil
		}
		if c.Writer.Header().Get("X-Prompt-Filter-History-Sanitized") != "" {
			body, err := io.ReadAll(c.Request.Body)
			if err != nil {
				return newResponsesWSInvalidRequestError(err)
			}
			create.Body = body
			if err := common.Unmarshal(body, &create.Request); err != nil {
				return newResponsesWSInvalidRequestError(err)
			}

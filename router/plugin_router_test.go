...
func TestProductionPluginNativeQueryTraversesInnerRouter(t *testing.T) {
	...
	require.NoError(t, err)
	require.NoError(t, database.AutoMigrate(&model.Task{}, &model.Log{}, &model.SystemDailyUsageCounter{}))
	...
	authenticatedProductionHandlers := func(
		...
		production := productionPluginRouteHandlers(generation, binding)
		handlers := append([]gin.HandlerFunc(nil), production...)
		handlers[3] = func(c *gin.Context) {
...
func TestWebFallbackDoesNotCacheMissingAPIOrAssets(t *testing.T) {
	...
	assert.Contains(t, page.Body.String(), "window.__NEW_API_AVAILABILITY__")

...
func SetTaskRouter(router *gin.Engine) {
	...
	taskSubmitRouter.Use(middleware.RouteTag("relay"), middleware.SystemAvailabilityCheck(), middleware.SystemDailyUsageLimitCheck(), middleware.TokenAuth())
	{
		taskSubmitRouter.POST("/:key", middleware.PrepareTaskPluginSubmit(), middleware.Distribute(), middleware.PromptComplianceCheck(""), controller.RelayTask)
	}

	taskReadRouter := router.Group("/v1/tasks")
	taskReadRouter.Use(middleware.RouteTag("relay"), middleware.SystemAvailabilityCheck(), middleware.SystemDailyUsageLimitCheck(), middleware.TokenAuth())
	{

...
test.each([
  ...
  'keeps Chinese weekday labels for %s and after a language switch',
...
test('builds weekday peak pricing with two time ranges from an empty visual form', async () => {
  ...
  await user.click(screen.getByRole('option', { name: '星期一' }))
  ...
  await user.click(screen.getByRole('option', { name: '星期五' }))
  await user.click(
...
test('updates weekday, hour and timezone conditions while preserving the other branches', async () => {
  ...
  await user.click(screen.getByRole('option', { name: '星期二' }))
  expect(onBillingExprChange).toHaveBeenLastCalledWith(

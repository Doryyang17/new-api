...
const factorLabels: Record<string, string> = {
  ...
  repeated_content_high: '重复内容较多（辅助）',
  repeated_content: '重复内容偏多（辅助）',
  model_sweep_high: '批量轮询模型（高）',
  model_sweep: '批量轮询模型',
  failure_burst_high: '历史规则：失败请求突增（已停用）',
  failure_burst: '历史规则：失败请求偏多（已停用）',
  fast_failure_retry: '历史规则：失败后快速重试（已停用）',

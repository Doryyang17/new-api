...
async function renderPolicies(path: string) {
  ...
  const routes = [
    createRoute({
      getParentRoute: () => authenticated,
      path: '/system-settings/custom/$section',
      component: () => <div>定制化功能</div>,
    }),
    policyRoute,
    createRoute({
      getParentRoute: () => authenticated,
...
describe('request policy settings', () => {
  it.each([
    ...
    async (section, saveLabel) => {
      await renderPolicies(
        section === 'custom/prompt-filter'
          ? '/system-settings/custom/prompt-filter'
  ...
  it.each([
    ...
    ['/system-settings/security/sensitive-words', 'custom/prompt-filter'],

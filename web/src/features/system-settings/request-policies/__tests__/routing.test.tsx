...
it.each([
  ...
  'renders cache statistics and TTL with the project language code %s and keeps Chinese formatting after a language switch',
  async (language, capacity, ttl) => {
    ...
    await waitFor(() => expect(status).toHaveTextContent(/2\s*\/\s*100,000/))
    expect(
      within(table).getByRole('cell', { name: '3,600 seconds' })
    ).toBeVisible()
    expect(api.patch).not.toHaveBeenCalled()
  }
)

...
const FIXED_TIER_PRICE_REGEX = /^-?(?:\d+\.?\d*|\.\d+)(?:[eE][+-]?\d+)?$/
...
export type ParsedTier = {
  ...
  /** Fixed per-request price in USD for a numeric-only tier body. */
...
function parseTierBody(bodyStr: string): Record<string, number> {
  ...
  if (FIXED_TIER_PRICE_REGEX.test(fixedPrice)) {
    tier.fixedPriceUSD = Number(fixedPrice) / 1_000_000
  }
  return tier
}

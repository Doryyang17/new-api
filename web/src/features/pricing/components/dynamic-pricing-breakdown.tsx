...
function getTierDisplayKey(tier: BreakdownTier): string {
  if (isTaskBreakdownTier(tier)) return JSON.stringify(tier)
  ...
  return `${tier.label}|${conditions}|${String(isTaskBreakdownTier(tier) ? tier.constant : tier.fixedPriceUSD || '')}|${prices}`
}
...
export function DynamicPricingBreakdown({
  ...
  const { tiers, ruleGroups } = useMemo(() => {
    ...
    if (matrixTiers) {
      ...
      parsedTiers = movePreferredTierFirst(
  ...
  }, [expr, matchedTierLabel, usageSchema, requestRules])
  ...
  const visiblePriceFields: BreakdownPriceField[] = (() => {
    ...
    if (
      tiers.some(
        (tier) =>
          !isTaskBreakdownTier(tier) && Number(tier.fixedPriceUSD || 0) > 0
      )
    ) {
      fields.push({
        id: 'fixed-price',
        ...
        unit: 'request',
        value: (tier) =>
          isTaskBreakdownTier(tier) ? 0 : Number(tier.fixedPriceUSD || 0),
      })
    }
    return fields
  })()

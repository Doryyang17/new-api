...
describe('log cost display', () => {
  ...
  test('keeps check-in bonus and probe markers alongside a tool surcharge', async () => {
    const rendered = await renderCost({
      quota: 2000,
      other: {
        consume_total: 5000,
        checkin_bonus_deducted: 3000,
        billing_mode: 'tiered_expr',
        matched_tier: '测活请求',
        tool_surcharges: [{ name: 'web_search', count: 1, price: 5 }],
      },
    })
    ...
    assert.ok(rendered.container.querySelector('img[alt="签到赠金抵扣"]'))
    assert.ok(rendered.container.querySelector('[aria-label="测活惩罚扣费"]'))
    assert.ok(
      rendered.container.querySelector('[data-tool-surcharge-indicator="true"]')
    )

...
export function getTieredBillingSummary(
  ...
  if (!tier) return null
  // Legacy numeric tiers use millionths of USD rather than fixed()'s USD unit.
  if (tier.fixedPriceUSD !== undefined) {
    return { tiers, tier, priceEntries: [], fixedPriceUSD: tier.fixedPriceUSD }
  }

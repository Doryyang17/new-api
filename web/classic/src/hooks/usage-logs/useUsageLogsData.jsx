...
export const useLogsData = () => {
  ...
  const setLogsFormat = (logs) => {
    ...
    for (let i = 0; i < logs.length; i++) {
      ...
      if (
        logs[i].type === 2 &&
        (other?.consume_total !== undefined ||
          other?.checkin_bonus_deducted !== undefined ||
          other?.original_funding_deducted !== undefined)
      ) {
        const total = other?.consume_total ?? logs[i].quota ?? 0;
        const bonus = other?.checkin_bonus_deducted ?? 0;
        const original = other?.original_funding_deducted ?? 0;
        const originalLabel =
        ...
        expandDataLocal.push({
          ...
          value: (
            <div style={{ whiteSpace: 'pre-line' }}>
              {`本次消费：${renderQuota(total, 6)}\n签到赠金抵扣：${renderQuota(bonus, 6)}\n${originalLabel}：${renderQuota(original, 6)}`}
            </div>
          ),
        });
      }

...
func appendBillingInfo(relayInfo *relaycommon.RelayInfo, other *model.LogOther) {
	...
	if consumeTotal > 0 {
		other.SetPublic("consume_total", consumeTotal)
	}
	if relayInfo.CheckinBonusConsumed > 0 {
		other.SetPublic("checkin_bonus_deducted", relayInfo.CheckinBonusConsumed)
	}
	if relayInfo.OriginalFundingConsumed > 0 {
		other.SetPublic("original_funding_deducted", relayInfo.OriginalFundingConsumed)
	}
	if relayInfo.BillingSource == "subscription" {
		...
		other.SetPublic("wallet_quota_deducted", relayInfo.OriginalFundingConsumed)
	}
}

func appendUserLevelBillingInfo(info hosttypes.GroupRatioInfo, other *model.LogOther) {
	if other == nil {
		return
	}
	...
	other.SetPublic("base_group_ratio", info.BaseGroupRatio)
	other.SetPublic("user_level_id", info.UserLevelID)
	other.SetPublic("user_level_name", info.UserLevelName)
	other.SetPublic("user_level_ratio", info.UserLevelRatio)
	other.SetPublic("user_level_color", info.UserLevelColor)
}

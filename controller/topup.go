...
func prepareTopUpCreditedQuotaForPayment(c *gin.Context, topUp *model.TopUp) bool {
	if err := model.PrepareTopUpCreditedQuota(topUp); err != nil {
		logger.LogWarn(c.Request.Context(), fmt.Sprintf(
			"充值订单额度校验失败 user_id=%d trade_no=%s payment_provider=%s amount=%d money=%.2f error=%q",
			topUp.UserId, topUp.TradeNo, topUp.PaymentProvider, topUp.Amount, topUp.Money, err.Error(),
		))
		c.JSON(http.StatusOK, gin.H{"message": "error", "data": "充值额度无效或超出系统支持范围"})
		return false
	}
	return true
}
...
func RequestEpay(c *gin.Context) {
	...
	if !prepareTopUpCreditedQuotaForPayment(c, topUp) {
		return
	}
...
func EpayNotify(c *gin.Context) {
	...
	if verifyInfo.TradeStatus == epay.StatusTradeSuccess {
		...
		topUp, quotaToAdd, completed, err := model.CompleteEpayTopUp(verifyInfo.ServiceTradeNo, verifyInfo.Type)
		...
		if err != nil {
			logger.LogError(c.Request.Context(), fmt.Sprintf("易支付 充值处理失败 trade_no=%s callback_type=%s client_ip=%s error=%q topup=%q", verifyInfo.ServiceTradeNo, verifyInfo.Type, c.ClientIP(), err.Error(), common.GetJsonString(topUp)))
			return
		}
		if completed {

...
func handleOAuthLogin(c *gin.Context, provider oauth.Provider, oauthUser *oauth.OAuthUser, token *oauth.OAuthToken, flow *model.AuthFlow) {
	...
	if err != nil {
		...
		case *OAuthRegistrationCodeRequiredError:
			storeOAuthPendingRegistration(c, flow.Provider, provider, oauthUser, payload.AffiliateCode)
...
func findExistingOAuthUser(c *gin.Context, provider oauth.Provider, oauthUser *oauth.OAuthUser, token *oauth.OAuthToken) (*model.User, *service.LegacyGitHubMigration, error) {
	user := &model.User{}
...
func findOrCreateOAuthUser(c *gin.Context, provider oauth.Provider, oauthUser *oauth.OAuthUser, token *oauth.OAuthToken, affiliateCode string) (*model.User, *service.LegacyGitHubMigration, error) {
	if user, migration, err := findExistingOAuthUser(c, provider, oauthUser, token); err != nil || user != nil {
	...
	if common.RegistrationCodeRegisterEnabled {
		...
		if blocked {
			return nil, nil, &OAuthRegistrationRiskBlockedError{RetryAfter: retryAfter}
		}
		return nil, nil, &OAuthRegistrationCodeRequiredError{}
	}

	user, err := createOAuthUserForNewUser(provider, oauthUser, affiliateCode, "", registrationRiskKeys)
	return user, nil, err
}
...
func findOrCreateOAuthUserWithRegistrationCode(
	...
	if legacyID, _ := oauthUser.Extra["legacy_id"].(string); legacyID != "" && provider.IsUserIDTaken(legacyID) {

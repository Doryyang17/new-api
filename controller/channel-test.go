...
func testChannel(ctx context.Context, channel *model.Channel, testUserID int, testModel string, endpointType string, isStream bool) testResult {
	...
	if usageStatus.ShouldBlock() {
		...
		return testResult{
			localErr:    err,
			newAPIError: types.NewErrorWithStatusCode(err, types.ErrorCodeSystemDailyUsageExceeded, http.StatusForbidden, types.ErrOptionWithSkipRetry()),
		}
	}
	tik := time.Now()
	...
	if modelLimit, shouldBlock := usageStatus.ShouldBlockModel(testModel); shouldBlock {
		...
		return testResult{
			localErr:    err,
			newAPIError: types.NewErrorWithStatusCode(err, types.ErrorCodeModelDailyUsageExceeded, http.StatusForbidden, types.ErrOptionWithSkipRetry()),
		}
	}

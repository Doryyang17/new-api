...
func setupResponsesWSRequestTest(t *testing.T) (*model.User, *model.Token) {
	...
	require.NoError(t, db.AutoMigrate(&model.User{}, &model.Token{}, &model.CheckinBonus{}, &model.CheckinBonusUsage{}))
...
// Each create event must recheck fork policies even on an already-open socket.
func TestResponsesWebSocketForkPoliciesRejectBeforeBilling(t *testing.T) {
	for _, kind := range []string{"curfew", "prompt"} {
		t.Run(kind, func(t *testing.T) {
			fixture := newResponsesWSBillingTest(t, `tier("request", fixed(0.002))`, func(*websocket.Conn, *http.Request) { t.Error("blocked request reached upstream") })
			saved := config.GlobalConfig.ExportAllConfigs()
			enabled, prompt, words := setting.CheckSensitiveEnabled, setting.CheckSensitiveOnPromptEnabled, setting.SensitiveWords
			t.Cleanup(func() {
				require.NoError(t, config.GlobalConfig.LoadFromDB(saved))
				setting.CheckSensitiveEnabled, setting.CheckSensitiveOnPromptEnabled, setting.SensitiveWords = enabled, prompt, words
			})
			status, code := http.StatusForbidden, "system_curfew"
			if kind == "curfew" {
				now := time.Now().UTC()
				require.NoError(t, config.GlobalConfig.LoadFromDB(map[string]string{"availability_setting.enabled": "true", "availability_setting.timezone": "UTC", "availability_setting.unavailable_start": now.Add(-time.Hour).Format("15:04"), "availability_setting.unavailable_end": now.Add(time.Hour).Format("15:04")}))
			} else {
				setting.CheckSensitiveEnabled, setting.CheckSensitiveOnPromptEnabled = true, true
				setting.SensitiveWords = []string{"merge-blocked-phrase"}
				require.NoError(t, config.GlobalConfig.LoadFromDB(map[string]string{"prompt_filter_setting.mode": "block", "prompt_filter_setting.threshold": "50", "prompt_filter_setting.block_status_code": "460", "prompt_filter_setting.block_error_code": "prompt_blocked", "prompt_filter_setting.group_whitelist": "[]"}))
				status, code = 460, "prompt_blocked"
			}
			require.NoError(t, fixture.client.WriteMessage(websocket.TextMessage, []byte(`{"type":"response.create","event_id":"blocked","stream_id":"planner","model":"ws-billing","input":"merge-blocked-phrase"}`)))
			event := readResponsesWSTestEvent(t, fixture.client)
			assert.Equal(t, float64(status), event["status"])
			assert.Equal(t, code, event["error"].(map[string]any)["code"])

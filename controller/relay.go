...
func Relay(c *gin.Context, relayFormat types.RelayFormat) {
	...
	defer func() {
		if newAPIError != nil {
			if middleware.RequestProtectionBlocked(c) && relayFormat != types.RelayFormatOpenAIRealtime {
				return
			}
	...
	if newAPIError = relay.PrepareRequestBilling(c, relayInfo); newAPIError != nil {
		if middleware.RequestProtectionBlocked(c) && relayFormat != types.RelayFormatOpenAIRealtime {
			middleware.WriteRequestProtectionResponse(c, middleware.ApplyRequestProtection(c))
		}
		return
	}
	defer func() {
...
func executeTaskSubmissionWith(
	...
	for ; retryParam.GetRetry() <= common.RetryTimes; retryParam.IncreaseRetry() {
		...
		if middleware.RequestProtectionBlocked(c) {
			return nil, service.TaskErrorWrapperLocal(errors.New("request blocked"), "request_protection_blocked", c.Writer.Status())
		}
		if requestErr := c.Request.Context().Err(); requestErr != nil {
	...
	task.PrivateData.CheckinBonusConsumed = min(relayInfo.CheckinBonusConsumed, result.Quota)
	task.PrivateData.TokenId = relayInfo.TokenId
	task.PrivateData.NodeName = common.NodeName
	task.PrivateData.BillingContext = &model.TaskBillingContext{
		SnapshotVersion: 1,
		BaseGroupRatio:  relayInfo.PriceData.GroupRatioInfo.BaseGroupRatio,
		UserLevelID:     relayInfo.PriceData.GroupRatioInfo.UserLevelID,
		UserLevelName:   relayInfo.PriceData.GroupRatioInfo.UserLevelName,
		UserLevelRatio:  relayInfo.PriceData.GroupRatioInfo.UserLevelRatio,

...
// requestGuardLogOther adapts the fork's existing request-guard metadata to the
// shared role-scoped log contract, keeping full prompt data administrator-only.
func requestGuardLogOther(values map[string]interface{}) *LogOther {
	other := NewLogOther()
	for key, value := range values {
		switch key {
		case "admin_info":

...
func RechargeEpay(tradeNo string, actualPaymentMethod string, callerIp string) (alreadyDone bool, err error) {
	...
	err = DB.Transaction(func(tx *gorm.DB) error {
		...
		quotaToAdd, calculationErr = calculateTopUpCreditedQuota(topUp)
		if calculationErr != nil {
			return calculationErr
		}
		topUp.CreditedQuota = int64(quotaToAdd)
		...
		return increaseUserQuotaTx(tx, topUp.UserId, quotaToAdd)
	})
	if err != nil {
...
func Recharge(referenceId string, customerId string, callerIp string) (err error) {
	...
	err = DB.Transaction(func(tx *gorm.DB) error {
		...
		return creditTopUpQuota(tx, topUp.UserId, quotaToAdd, map[string]interface{}{
...
func RechargeCreem(referenceId string, customerEmail string, customerName string, callerIp string) (err error) {
	...
	err = DB.Transaction(func(tx *gorm.DB) error {
		...
		return creditTopUpQuota(tx, topUp.UserId, quotaToAdd, updateFields)
	})

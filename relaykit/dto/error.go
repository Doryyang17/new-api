...
func (e GeneralErrorResponse) TryToOpenAIError() *types.OpenAIError {
	...
	if len(e.Error) > 0 {
		err := kitutil.Unmarshal(e.Error, &structuredError)
		if err == nil && structuredError.Message != "" {
			if kitutil.GetJsonType(structuredError.Status) == "string" {
				var upstreamStatus string
				if err := kitutil.Unmarshal(structuredError.Status, &upstreamStatus); err == nil {
